...
async fn test_correct_recursive_extraction() -> TestResult {
    ...
    let outer_schema = SchemaExt::parse_str(raw_outer_schema).await?;
...
#[tokio::test]
async fn test_parse() -> TestResult {
    init();
...
async fn test_valid_cast_to_string_after_parse() -> TestResult {
    init();
    for (raw_schema, _) in valid_examples().iter() {
        let schema = SchemaExt::parse_str(raw_schema).await?;
        SchemaExt::parse_str(schema.canonical_form().as_str()).await?;
    }
    Ok(())
}
...
async fn test_parse_list_without_cross_deps() -> TestResult {
    init();
    let schema_str_1 = r#"{
        "name": "A",
        "type": "record",
    ...
    let schemas = SchemaExt::parse_list(schema_strs).await?;

    for schema_str in &schema_strs {
        let parsed = SchemaExt::parse_str(schema_str).await?;
...
async fn test_parse_list_with_cross_deps_basic() -> TestResult {
    init();
    let schema_a_str = r#"{
        "name": "A",
        "type": "record",
    ...
    let schemas_first = SchemaExt::parse_list(schema_strs_first).await?;
    let schemas_second = SchemaExt::parse_list(schema_strs_second).await?;
...
#[tokio::test]
async fn test_parse_list_recursive_type() -> TestResult {
    init();
    let schema_str_1 = r#"{
        "name": "A",
        "doc": "A's schema",
    ...
    let _ = SchemaExt::parse_list(schema_strs_first).await?;
    let _ = SchemaExt::parse_list(schema_strs_second).await?;
    Ok(())
}

#[tokio::test]
/// Test that schema composition resolves namespaces.
async fn test_parse_list_with_cross_deps_and_namespaces() -> TestResult {
    init();
    let schema_a_str = r#"{
        "name": "A",
        "type": "record",
    ...
    let schemas_first = SchemaExt::parse_list([schema_a_str, schema_b_str]).await?;
    let schemas_second = SchemaExt::parse_list([schema_b_str, schema_a_str]).await?;
...
async fn test_parse_list_with_cross_deps_and_namespaces_error() -> TestResult {
    init();
    let schema_str_1 = r#"{
        "name": "A",
        "type": "record",
...
async fn test_parse_reused_record_schema_by_fullname() -> TestResult {
    init();
    let schema_str = r#"
        {
          "type" : "record",
...
async fn test_parse_list_multiple_dependencies() -> TestResult {
    init();
    let schema_a_str = r#"{
        "name": "A",
        "type": "record",
    ...
    let parsed = SchemaExt::parse_list([schema_a_str, schema_b_str, schema_c_str]).await?;
    ...
    for schema_str_perm in permutations(&schema_strs) {
        ...
        let schemas = SchemaExt::parse_list(&schema_str_perm).await?;
...
/// definitions are passed in as a list. This should work regardless of the ordering of the list.
async fn test_parse_list_shared_dependency() -> TestResult {
    init();
    let schema_a_str = r#"{
        "name": "A",
        "type": "record",
...
async fn test_name_collision_error() -> TestResult {
    init();
    let schema_str_1 = r#"{
        "name": "foo.A",
        "type": "record",
    ...
    let _ = SchemaExt::parse_list([schema_str_1, schema_str_2])
        .await
        .expect_err("Test failed");
    Ok(())
}
...
async fn test_namespace_prevents_collisions() -> TestResult {
    init();
    let schema_str_1 = r#"{
        "name": "A",
        "type": "record",
    ...
    let parsed = SchemaExt::parse_list([schema_str_1, schema_str_2]).await?;
    let parsed_1 = SchemaExt::parse_str(schema_str_1).await?;
    let parsed_2 = SchemaExt::parse_str(schema_str_2).await?;
...
#[tokio::test]
async fn test_fullname_name_and_namespace_specified() -> TestResult {
    init();
    let name: Name =
...
#[tokio::test]
async fn test_fullname_fullname_and_namespace_specified() -> TestResult {
    init();
...
#[tokio::test]
async fn test_fullname_name_and_default_namespace_specified() -> TestResult {
    init();
...
#[tokio::test]
async fn test_fullname_fullname_and_default_namespace_specified() -> TestResult {
    init();
...
#[tokio::test]
async fn test_avro_3452_parsing_name_without_namespace() -> TestResult {
    init();
...
#[tokio::test]
async fn test_avro_3452_parsing_name_with_leading_dot_without_namespace() -> TestResult {
    init();
...
#[tokio::test]
async fn test_avro_3452_parse_json_without_name_field() -> TestResult {
    init();
...
#[tokio::test]
async fn test_fullname_fullname_namespace_and_default_namespace_specified() -> TestResult {
    init();
    let name: Name =
...
#[tokio::test]
async fn test_fullname_name_namespace_and_default_namespace_specified() -> TestResult {
    init();
    let name: Name =
...
#[tokio::test]
async fn test_doc_attributes() -> TestResult {
    init();
    ...
    for (raw_schema, _) in DOC_EXAMPLES.iter() {
        let original_schema = SchemaExt::parse_str(raw_schema).await?;
...
#[tokio::test]
async fn test_other_attributes() {
...
#[tokio::test]
async fn test_root_error_is_not_swallowed_on_parse_error() -> Result<(), String> {
    init();
    let raw_schema = r#"/not/a/real/file"#;
    let error = SchemaExt::parse_str(raw_schema)
        .await
        .unwrap_err()
        .into_details();
...
// AVRO-3302
#[tokio::test]
async fn test_record_schema_with_cyclic_references() -> TestResult {
    init();
    ...
    if let Err(err) = writer.append(datum).await {
        panic!("An error occurred while writing datum: {err:?}")
    }
    let bytes = writer.into_inner().await?;
    assert_eq!(316, bytes.len());

    let data = &mut bytes.as_slice();
    let reader = Reader::new(data).await;
    match reader {
        Ok(mut reader) => match reader.next().await {
...
#[tokio::test]
async fn test_decimal_valid_type_attributes() {
    init();
    let fixed_decimal = SchemaExt::parse_str(DECIMAL_LOGICAL_TYPE_ATTRIBUTES[0]).await?;
    ...
    let bytes_decimal = SchemaExt::parse_str(DECIMAL_LOGICAL_TYPE_ATTRIBUTES[1]).await?;
...
#[tokio::test]
async fn avro_old_issue_47() -> TestResult {
    init();
    let schema_str = r#"
    {
      "type": "record",
    ...
    let serialized_bytes = to_avro_datum(&schema, ser_value).await?;

    let de_value = &from_avro_datum(&schema, &mut &*serialized_bytes, None).await?;
...
#[tokio::test]
async fn test_avro_3785_deserialize_namespace_with_nullable_type_containing_reference_type()
-> TestResult {
    use serde::{Deserialize, Serialize};
    ...
    let writer_schema = SchemaExt::parse_str(writer_schema).await?;
    let foo1 = Foo {
        bar_init: Bar::Bar0,
    ...
    let mut x = &datum[..];
    let reader_schema = SchemaExt::parse_str(reader_schema).await?;
...
#[tokio::test]
async fn test_avro_3847_union_field_with_default_value_of_ref() -> TestResult {
    ...
    "#;
    let writer_schema = SchemaExt::parse_str(writer_schema_str).await?;
    ...
    "#;
    let reader_schema = SchemaExt::parse_str(reader_schema_str).await?;
...
#[tokio::test]
async fn test_avro_3847_union_field_with_default_value_of_ref_with_namespace() -> TestResult {
    // Test for reference to Record
    let writer_schema_str = r#"
    {
        "name": "record1",
...
#[tokio::test]
async fn test_avro_3847_union_field_with_default_value_of_ref_with_enclosing_namespace()
-> TestResult {
    // Test for reference to Record
    let writer_schema_str = r#"
    {
...
async fn write_schema_for_default_value_test() -> apache_avro::AvroResult<Vec<u8>> {
    let writer_schema_str = r#"
    {
        "name": "record1",
        "namespace": "ns",
...
#[tokio::test]
async fn test_avro_3851_read_default_value_for_simple_record_field() -> TestResult {
    let reader_schema_str = r#"
    {
        "name": "record1",
        "namespace": "ns",
    ...
    "#;
    let reader_schema = SchemaExt::parse_str(reader_schema_str).await?;
    let input = write_schema_for_default_value_test().await?;
    let mut reader = Reader::with_schema(&reader_schema, &input[..]).await?;
    let mut result = Vec::new();
    while let Some(value) = reader.next().await {
        match value {
...
#[tokio::test]
async fn test_avro_3851_read_default_value_for_nested_record_field() -> TestResult {
    let reader_schema_str = r#"
    {
        "name": "record1",
        "namespace": "ns",
...
#[tokio::test]
async fn test_avro_3851_read_default_value_for_enum_record_field() -> TestResult {
    let reader_schema_str = r#"
    {
        "name": "record1",
        "namespace": "ns",
...
#[tokio::test]
async fn test_avro_3851_read_default_value_for_fixed_record_field() -> TestResult {
    let reader_schema_str = r#"
    {
        "name": "record1",
        "namespace": "ns",
...
#[tokio::test]
async fn test_avro_3851_read_default_value_for_array_record_field() -> TestResult {
    let reader_schema_str = r#"
    {
        "name": "record1",
        "namespace": "ns",
...
#[tokio::test]
async fn test_avro_3851_read_default_value_for_map_record_field() -> TestResult {
    let reader_schema_str = r#"
    {
        "name": "record1",
        "namespace": "ns",
...
#[tokio::test]
async fn test_avro_3851_read_default_value_for_ref_record_field() -> TestResult {
    let writer_schema_str = r#"
    {
        "name": "record1",
        "namespace": "ns",
...
#[tokio::test]
async fn test_avro_3851_read_default_value_for_enum() -> TestResult {
    let writer_schema_str = r#"
    {
        "name": "enum1",
        "namespace": "ns",
...
#[tokio::test]
async fn avro_rs_66_test_independent_canonical_form_primitives() -> TestResult {
    init();
    ...
    let independent_schema = SchemaExt::parse_str(record_with_no_dependencies).await?;
    let schema_strs = [
        fixed_primitive,
        enum_primitive,
        record_primitive,
    ...
    for schema_str_perm in permutations(&schema_strs) {
        let schema_str_perm: Vec<&str> = schema_str_perm.iter().map(|s| **s).collect();
        let schemata = SchemaExt::parse_list(&schema_str_perm).await?;
...
#[tokio::test]
async fn avro_rs_66_test_independent_canonical_form_usages() -> TestResult {
    init();
    let record_primitive = r#"{
        "name": "Rec",
        "namespace": "ns",
            ...
            match schema.name().unwrap().to_string().as_str() {
                "RecUsage" => {
                    assert_eq!(
                        ...
                        SchemaExt::parse_str(record_usage_independent)
                            .await?
                            .canonical_form()
                    );
                }
                "ArrayUsage" => {
                    assert_eq!(
                        schema.independent_canonical_form(&schemata)?,
                        SchemaExt::parse_str(array_usage_independent)
                            .await?
                            .canonical_form()
                    );
                }
                "UnionUsage" => {
                    assert_eq!(
                        schema.independent_canonical_form(&schemata)?,
                        SchemaExt::parse_str(union_usage_independent)
                            .await?
                            .canonical_form()
                    );
                }
                "MapUsage" => {
                    assert_eq!(
                        schema.independent_canonical_form(&schemata)?,
                        SchemaExt::parse_str(map_usage_independent)
                            .await?
                            .canonical_form()
                    );
                }
...
#[tokio::test]
async fn avro_rs_66_test_independent_canonical_form_deep_recursion() -> TestResult {
    init();
    let record_primitive = r#"{
        "name": "Rec",
        "namespace": "ns",
    ...
    for schema_str_perm in permutations(&schema_strs) {
        ...
        assert_eq!(
            ...
            SchemaExt::parse_str(record_usage_usage_independent)
                .await?
                .canonical_form()
        );
    }
...
#[tokio::test]
async fn avro_rs_66_test_independent_canonical_form_missing_ref() -> TestResult {
    init();
    let record_primitive = r#"{
        "name": "Rec",
        "namespace": "ns",
    ...
    let schemata = SchemaExt::parse_list(schema_strs).await?;
    assert!(matches!(
        schemata[1]
