...
mod decode {
    ...
    use crate::{
        bigdecimal::asynch::deserialize_big_decimal,
        decimal::Decimal,
        duration::Duration,
        error::Details,
        error::Error,
        ...
        safe_len(
            usize::try_from(match raw_len.cmp(&0) {
                ...
                std::cmp::Ordering::Less => {
                    let _size = zag_i64(reader).await?;
    ...
    pub(crate) async fn decode_internal<R: AvroRead + Unpin, S: Borrow<Schema>>(
        ...
        match *schema {
            ...
            Schema::Uuid => {
                let Value::Bytes(bytes) = Box::pin(decode_internal(
                    &Schema::Bytes,
                    names,
                    enclosing_namespace,
                    reader,
            ...
            Schema::Array(ref inner) => {
                ...
                loop {
                    let len = decode_seq_len(reader).await?;
                    if len == 0 {
                        break;
                    }
            ...
            Schema::Union(ref inner) => match zag_i64(reader).await.map_err(Error::into_details) {
                Ok(index) => {
            ...
            Schema::Enum(EnumSchema { ref symbols, .. }) => {
                Ok(if let Value::Int(raw_index) = decode_int(reader).await? {
    ...
    mod tests {
        ...
        use crate::{
            decimal::Decimal,
            decode::asynch::decode,
            encode::asynch::{encode, tests::success},
                ...
                r#"
        {
            ...
            encode(&outer_value2, &schema, &mut buf)
                .await
                .expect(&success(&outer_value2, &schema));
            let mut bytes = &buf[..];
            assert_eq!(
                outer_value2,
                decode(&schema, &mut bytes).await.expect(&format!(
        ...
        #[tokio::test]
        async fn test_avro_3448_proper_multi_level_decoding_middle_namespace() -> TestResult {
            // if encoding fails in this test check the corresponding test in encode
            let schema = r#"
        {
          "name": "record_name",
        ...
        #[tokio::test]
        async fn test_avro_3448_proper_multi_level_decoding_inner_namespace() -> TestResult {
            // if encoding fails in this test check the corresponding test in encode
            let schema = r#"
        {
          "name": "record_name",
