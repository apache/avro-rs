...
use crate::{
    ...
    decode::decode_len,
    error::Details,
    schema::{DecimalSchema, InnerDecimalSchema, NamesRef, SchemaKind, UuidSchema},
    serde::deser_schema::{
...
impl<'de, 's, 'r, R: Read> Deserializer<'de> for SchemaAwareDeserializer<'s, 'r, R> {
        ...
        V: serde::de::Visitor<'de>,
    {
        ...
        let boolean = match buf[0] {
            0 => false,
            1 => true,
            _ => return Err(self.error("bool", format!("{} is not a valid boolean", buf[0]))),
        };
    ...
    where
        V: serde::de::Visitor<'de>,
    {
        if let Some(name) = self.schema.name() {
            visitor.visit_str(&name.name)
        } else {
            Err(self.error("identifier", "Expected a named schema"))
        }
    }
